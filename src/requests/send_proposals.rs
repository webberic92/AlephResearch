...
pub async fn send_proposals(
    ...
    let node_guard = match lock_result {
        Ok(guard) => guard,
        Err(_) => {
            error!("❌ Timeout acquiring node lock for send_proposals() in round {}", propose_request.base.round_id);
            return Err(anyhow!("Timeout acquiring lock"));
        }
    };
    ...
    let rbc_processor = node_guard.rbc_processor.clone();
    drop(node_guard); // release lock

    info!("🚀 Node {} preparing to send proposals to {:?} for round {}", node_id, nodes, round_id);
    ...
    if results.iter().all(|r| r.is_ok()) {
        info!("✅ All proposals sent successfully for round {}.", round_id);

        if let Some(rbc_processor) = rbc_processor {
            let msg = RBCMessage::Proposal(propose_request.clone());
            info!("📥 Enqueuing proposal into RBCProcessor for round {}", round_id);
            rbc_processor.enqueue_message(msg).await;
            Ok(())
        } else {
            error!("❌ RBCProcessor not initialized when enqueuing proposal.");
            Err(anyhow!("RBCProcessor not initialized"))
        }
    } else {
        Err(anyhow!("❌ One or more proposals failed after retries."))
    }
}
