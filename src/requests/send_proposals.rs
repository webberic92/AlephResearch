...
pub async fn send_proposals(
    ...
    match Node::update_proposal_tracker(node.clone(), propose_request.base.proposing_node_id).await {
        Ok((proposal_count, required_proposals)) => {
            info!(
                "Node {}: Added itself to proposal tracker. Proposal count: {} / Required: {}",
                propose_request.base.proposing_node_id, proposal_count, required_proposals
            );
