...
pub async fn handle_dag_sync(
    node: Arc<Mutex<Node>>, // Now uses Arc<Mutex<Node>> for consistency
    ...
    // Acquire a lock to access node properties safely
    ...
    match check_dag_sync(&client, payload.round_id, &payload.sender_id, &payload.sender_url).await {
        ...
        Err(e) => {
            error!(
                ...
                node_guard.id, payload.sender_id, payload.round_id, e
            );
            Json(DAGSyncResponse {
                in_sync: false,
